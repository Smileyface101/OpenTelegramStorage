...
DATA_DIR = Path(os.getenv("OTG_DATA_DIR", "./data")).resolve()
STAGING_DIR = DATA_DIR / "staging"
DB_PATH = DATA_DIR / "otg.db"
MASTER_KEY_FILE = DATA_DIR / "master.key"
MASTER_KEY_ENV = "OTG_MASTER_KEY"

# Cookie hardening. None = decide per request from the scheme (https -> Secure).
COOKIE_SECURE: bool | None = _env_bool("OTG_COOKIE_SECURE", None)
SESSION_TTL_DAYS = _env_int("OTG_SESSION_TTL_DAYS", 14)

# Reverse proxies in front of the app, counted from the right of
# X-Forwarded-For. 0 = trust the socket peer only.
TRUSTED_PROXY_COUNT = _env_int("OTG_TRUSTED_PROXY_COUNT", 0)
...
DEFAULT_PART_SIZE_MB = _env_int("OTG_DEFAULT_PART_SIZE_MB", 512)
MAX_UPLOAD_RETRIES = 3
STAGING_FREE_SPACE_MARGIN = 256 * 1024 * 1024

# Where the built SPA lives (Docker copies it here).
FRONTEND_DIST = Path(os.getenv("OTG_FRONTEND_DIST", str(Path(__file__).resolve().parents[2] / "frontend" / "dist")))
